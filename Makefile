...
# handle nostrip
ifeq (,$(findstring nostrip,$(HOTSTUFF_BUILD_OPTIONS)))
  BUILD_FLAGS += -trimpath
  # LD_FLAGS += -s -w
endif

# handle race
ifeq (race,$(findstring race,$(HOTSTUFF_BUILD_OPTIONS)))
  CGO_ENABLED=1
  BUILD_FLAGS += -race
endif

# handle cleveldb
ifeq (cleveldb,$(findstring cleveldb,$(HOTSTUFF_BUILD_OPTIONS)))
  CGO_ENABLED=1
  BUILD_TAGS += cleveldb
endif

# handle badgerdb
ifeq (badgerdb,$(findstring badgerdb,$(HOTSTUFF_BUILD_OPTIONS)))
  BUILD_TAGS += badgerdb
endif

# handle rocksdb
ifeq (rocksdb,$(findstring rocksdb,$(HOTSTUFF_BUILD_OPTIONS)))
  CGO_ENABLED=1
  BUILD_TAGS += rocksdb
endif

# handle boltdb
ifeq (boltdb,$(findstring boltdb,$(HOTSTUFF_BUILD_OPTIONS)))
  BUILD_TAGS += boltdb
endif
...
build:
	CGO_ENABLED=$(CGO_ENABLED) go build $(BUILD_FLAGS) -tags '$(BUILD_TAGS)' -o $(OUTPUT) ./cmd/hotstuff/
.PHONY: build

install:
	CGO_ENABLED=$(CGO_ENABLED) go install $(BUILD_FLAGS) -tags $(BUILD_TAGS) ./cmd/hotstuff
.PHONY: install
...
proto-gen: check-proto-deps
	...
	@mv ./proto/hotstuff/abci/types.pb.go ./abci/types/
.PHONY: proto-gen
...
# Runs `make build HOTSTUFF_BUILD_OPTIONS=cleveldb` from within an Amazon
