...
func (cs *State) newStep() {
	...
	if cs.eventBus != nil {
		...
		// 这个事件会 conR.broadcastNewRoundStepMessage(data.(*cstypes.RoundState))
		// 广播 RoundState
...
func (cs *State) handleMsg(mi msgInfo) {
	...
	case *VoteMessage:
		...
		if added {
			cs.statsMsgQueue <- mi // 用于统计？
		}

		// 这里有个 TODO，当 vote 有问题的时候 slash 投票者
		...
		// a typical peer.
		// https://github.com/xufeisofly/hotstuff/issues/1281
		// }
...
func (cs *State) handleTimeout(ti timeoutInfo, rs cstypes.RoundState) {
	...
	case cstypes.RoundStepPropose:
		...
		// ProposeTimeout 到时间也会触发 enterPrevote
...
func (cs *State) handleTxsAvailable() {
	...
	case cstypes.RoundStepNewHeight: // timeoutCommit phase
		...
		// StartTime 是 CommitTime + timeoutCommit，CommitTime 和 Now() 很接近，因此可以认为
		// NewHeight 和 NewRound 之间相隔 timeoutCommit
...
func (cs *State) enterNewRound(height int64, round int32) {
	...
	// 还没到 StartTime 就开启了 NewRound，需要额外延迟
...
func (cs *State) enterPropose(height int64, round int32) {
	...
	defer func() {
		...
		if cs.isProposalComplete() {
			// 判断一下，当已经收到完成的 Proposal 时，直接进入 Prevote 阶段（比如 Proposer 肯定直接进入）
			// 正常来说是要等待 Proposal 消息，或是等待 ProposeTimeout 才尝试进入
...
func (cs *State) enterPrevoteWait(height int64, round int32) {
	...
	// Wait for some more prevotes; enterPrecommitenterPrecommit
	// 开始计时，时间到了进入 enterPrecommit
...
func (cs *State) enterPrecommit(height int64, round int32) {
	...
	// 如果收集到 2/3 的 nil，则 Unlock polka，然后投票 nil Precommit
	...
	// 如果收集到 +2/3 的 block prevote 投票，则设置 LockBlock 或更新 LockedRound，并投票 block Precommit
	...
	// There was a polka in this round for a block we doen't have.
	// Fetch that block, unlock, and precommit nil.
	// The +2/3 prevotes for this round is the POL for our unlock.

	// 如果 +2/3 的 prevote 是一个不知名的 Block，那么 Unlock 之前的 LockBlock，然后投票 nil Precommit
...
func (cs *State) tryAddVote(vote *types.Vote, peerID p2p.ID) (bool, error) {
	...
	if err != nil {
		...
		if voteErr, ok := err.(*types.ErrVoteConflictingVotes); ok {
			...
			// 		(https://github.com/xufeisofly/hotstuff/issues/3839).
...
func (cs *State) addVote(vote *types.Vote, peerID p2p.ID) (added bool, err error) {
	...
	// 在 timeoutCommit 期间可能收到前一个 height 的 precommit 消息
	// 即，已经 commit 完成了，但是还在收集剩余的 precommit 消息
	if vote.Height+1 == cs.Height && vote.Type == tmproto.PrecommitType {
		if cs.Step != cstypes.RoundStepNewHeight {
			// Late precommit at prior height is ignored
			// 此时 commitTimeout 已经结束了
			cs.Logger.Debug("precommit vote came in after commit timeout and hasa341s been ignored", "vote", vote)
			return
		}

		// 记录收集到的剩余 1/3 的 precommit 消息
		...
		// 广播 HasVote 消息，告诉其他节点收到了某个投票？
		cs.evsw.FireEvent(types.EventVote, vote)

		// if we can skip timeoutCommit and have all the votes now,
		// 如果要跳过 CommitTimeout 就必须收集到所有的 precommit 投票！
		// ??? 作用是什么
		// 猜测：感觉是确保所有的 validator 都已经收到了足够的 prevote 投票，即 lock 了区块
		// 这样避免了下一轮的 leader 没有 lock 上一轮区块的尴尬，一旦发生这个问题，就会导致下一个区块共识失败
		// 直到将上一轮的区块提交
	...
	switch vote.Type {
	case tmproto.PrevoteType: // 收到了 prevote 投票
		prevotes := cs.Votes.Prevotes(vote.Round)
		cs.Logger.Debug("added vote to prevote", "vote", vote, "prevotes", prevotes.StringShort())

		// If +2/3 prevotes for a block or nil for *any* round:
		// prevote 投票凑够了（可能是 +2/3 的 block 也可能是 +2/3 的 nil）
		if blockID, ok := prevotes.TwoThirdsMajority(); ok {
			...
			// 针对新的 block 产生了一个 polka(+2/3 认可)，此时可以 Unlock 之前的 Block
			if (cs.LockedBlock != nil) &&
			...
			if len(blockID.Hash) != 0 && (cs.ValidRound < vote.Round) && (vote.Round == cs.Round) {
				if cs.ProposalBlock.HashesTo(blockID.Hash) {
					...
					// 产生了 ValidBlock 等信息
					// ??? 还是不理解，ValidBlock 和 LockedBlock 有什么区别?
					// ValidBlock 表示当前决定对 ValidBlock 进行共识，而不是重新打包一个区块
		...
		// 产生了 LockedQC 之后，尝试进入下一阶段，正常来说是 prevoteTimeout 阶段或是 precommit 阶段
		// If +2/3 prevotes for *anything* for future round:
		switch {
		case cs.Round < vote.Round && prevotes.HasTwoThirdsAny(): // 本地 round 过时了
			// Round-skip if there is any 2/3+ of votes ahead of us
			cs.enterNewRound(height, vote.Round)

		case cs.Round == vote.Round && cstypes.RoundStepPrevote <= cs.Step: // current round
			blockID, ok := prevotes.TwoThirdsMajority()
			// 收到了 +2/3 的 prevote 投票（block 投票或 nil 投票），可进入 precommit 阶段
			if ok && (cs.isProposalComplete() || len(blockID.Hash) == 0) {
				cs.enterPrecommit(height, vote.Round)
			} else if prevotes.HasTwoThirdsAny() {
				// 否则开始进入 prevote wait 阶段
				// 这个阶段就是开启了一个 timeout ticker，等待 handleTimeoutMsg 触发然后 enterPrecommit
	...
	case tmproto.PrecommitType: // 收到 Precommit 投票
		...
		if ok {
			...
			if len(blockID.Hash) != 0 { // 如果 +2/3 是同意票，则进入 Commit 阶段，提交提案
				cs.enterCommit(height, vote.Round)
				// 根据配置选择是否要等待 CommitTimeout 还是要收集所有的 precommits(避免新 Leader 没有提交？)
				if cs.config.SkipTimeoutCommit && precommits.HasAll() {
					// 进入全新的 Height，打包新区块
					cs.enterNewRound(cs.Height, 0)
				}
			} else {
				// 如果收集到了 +2/3 是 nil 票，则进入 precommitWait，再等一等
				// 会在 precommitTimeout 结束时发起 NewRound 的
				// precommitTimeout 主要用于视图切换???
...
func (cs *State) voteTime() time.Time {
	...
	// even if cs.LockedBlock != nil. See https://github.com/xufeisofly/hotstuff/tree/v0.34.x/spec/.
	timeIota := time.Duration(cs.state.ConsensusParams.Block.TimeIotaMs) * time.Millisecond
	if cs.LockedBlock != nil {
		// See the BFT time spec
		// https://github.com/xufeisofly/hotstuff/blob/v0.34.x/spec/consensus/bft-time.md
